...
public class VoxyRenderSystem {
    ...
    // Deferred pipeline update state: when the Iris pipeline changes mid-frame,
    // we must avoid destroying the viewportSelector/chunkBoundRenderer immediately
    // because the active viewport's HiZ texture would be freed mid-render.
    // Instead, we create new resources and swap them at a safe point.
    // The old pipeline must also be kept alive until the swap, since the old
    // chunkBoundRenderer references it.
